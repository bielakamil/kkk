...
<html>
<head>
    <title> MojePrawko.pl - wszyscy instruktorzy </title>
    <?php head (); ?>
</head>
...
<?php include_once("../script/analyticstracking.php") ?>
    ...
    <header class="all" id="head">
        <div id="top">            <i class="material-icons" onclick="show_menu()"> menu </i>
          ...
          <h1 class="add"> Wszystkie tankowania </h1>
        <?php
       if ($_SESSION['status'] == 3)
        {
            echo '<a href="../page/add_paliwo.php?id=' . $_GET['id'] . '">';
                echo '<div id="button_add">';
                    echo 'Dodaj tankowanie';
                echo '</div>';
            echo '</a>'; 
        ...
        <div id="text" class="all-page show">
                <?php
                    ...
                    if ($status == 1) {
                        echo $admin->show_car_paliwo($_GET['id']);
                    } elseif ($status == 2) {
                        $instruktor->show_car_paliwo($_GET['id']);
                    } 
