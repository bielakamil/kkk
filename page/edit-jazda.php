...
head ();      
?>
<title> MojePrawko.pl - edytuj jazdę </title>
</head>
<body>
    <?php 
                    ...
                    <div class="row">
                        <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12 bottom">
                            <?php
                                if ($status == 1) 
                                {
                                    $admin->edit_jazda($_GET['id']);
                                } elseif ($status == 2) 
                                {
                                    $instruktor->edit_jazda($_GET['id']);
                                }
                            ?>
                        </div>
                    </div>
...
<script>
    ...
    $('#stan').change(function ()
    {
        ...
        console.log ("Stan " + value);
        
        if (value == 3)
        {
            console.log ("Wyświetl karę");
            
            $('.kara').css('display','block');
